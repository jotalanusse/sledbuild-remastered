...
function MAP.GatesOpen()
	for k, v in pairs(ents.FindByName(ENTITY.NAMES.GATE_NAME)) do
...
function MAP.PushersEnable()
	for k, v in pairs(ents.FindByName(ENTITY.NAMES.PUSHER_NAME)) do
...
function MAP.SelectRandomSpawn()
	local spawns = ents.FindByName(ENTITY.NAMES.SPAWN_NAME)
	local selectedSpawnIndex = math.random(1, MAP.CountByName(ENTITY.NAMES.SPAWN_NAME))
