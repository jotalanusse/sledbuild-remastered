...
function MCHK.CheckMap(ply)
  ...
  if (totalStartZones == 0) then
    NET.SendGamemodeMessage(ply, "ERROR: No start zones found!", CONSOLE.ERROR_COLOR)
    completeMap = false
  end

  local totalFinishZones = MAP.CountByClass(ENTITY_NAMES.FINISH_ZONE_CLASS)
  if (totalFinishZones == 0) then
    NET.SendGamemodeMessage(ply, "ERROR: No finish zones found!", CONSOLE.ERROR_COLOR)
    completeMap = false
  end

  local totalGates = MAP.CountByName(ENTITY_NAMES.GATE_NAME)
  if (totalGates == 0) then
    NET.SendGamemodeMessage(ply, "ERROR: No gates found!", CONSOLE.ERROR_COLOR)
    completeMap = false
  end

  local totalPushers = MAP.CountByName(ENTITY_NAMES.PUSHER_NAME)
  if (totalGates == 0) then
    NET.SendGamemodeMessage(ply, "ERROR: No pushers found!", CONSOLE.ERROR_COLOR)
    completeMap = false
  end

  local totalSpawns1 = MAP.CountByName(ENTITY_NAMES.SPAWN_1_NAME)
  if (totalSpawns1 == 0) then
    NET.SendGamemodeMessage(ply, "ERROR: No specific spawn 1 found!", CONSOLE.ERROR_COLOR)
    completeMap = false
  end

  local totalSpawns2 = MAP.CountByName(ENTITY_NAMES.SPAWN_2_NAME)
  if (totalSpawns2 == 0) then
    NET.SendGamemodeMessage(ply, "ERROR: No specific spawn 2 found!", CONSOLE.ERROR_COLOR)
    completeMap = false
  end

  local totalSpawns3 = MAP.CountByName(ENTITY_NAMES.SPAWN_3_NAME)
  if (totalSpawns3 == 0) then
    NET.SendGamemodeMessage(ply, "ERROR: No specific spawn 3 found!", CONSOLE.ERROR_COLOR)
    completeMap = false
  end

  local totalSpawns = MAP.CountByName(ENTITY_NAMES.SPAWN_NAME)
  if (totalSpawns == 0) then
    NET.SendGamemodeMessage(ply, "ERROR: No additional spawns found!", CONSOLE.ERROR_COLOR)
    completeMap = false
  end

  if (not completeMap) then
    NET.SendGamemodeMessage(ply, "ERROR: The map is not complete, gameplay might be broken!", CONSOLE.ERROR_COLOR)
    NET.SendGamemodeMessage(ply, "ERROR: Please check the map for missing entities!", CONSOLE.ERROR_COLOR)
