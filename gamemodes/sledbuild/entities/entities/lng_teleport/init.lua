...
function ENT:StartTouch(entity)
	...
	if entity:IsPlayer() and not entity:InVehicle() then
		-- The player isn't supposed to get here without a vehicle.. kill him! >:-D
		if not game.GetMap() == "slb_snowsled_v3" then -- DON'T PUT TRIGGERS OVER NON-RACING AREAS!
			entity:Kill()
		end
	end
	...
	elseif (entity:IsValid() and (entity:GetClass() == "prop_vehicle_prisoner_pod")) then -- The sled has no player in it! (but a vehicle!)
